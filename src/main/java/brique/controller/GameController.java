...
public class GameController {
    ...
    public boolean winBoard() {
        if (goalWinner(turns.firstPlayer())) return true;
        return goalWinner(turns.secondPlayer());
    }
    ...
    private boolean goalWinner(Player player) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (board.boundsWithin(row, col) &&
                    ((col == board.getCols() - 1 && player.equals(turns.secondPlayer())) ||
                            (row == board.getRows() - 1 && player.equals(turns.firstPlayer())))) {
                return true;
            }
